...
use boson::{
    did::{
        constants, w3c::VerifiableCredential as VC, w3c::VerifiablePresentation as VP, DIDUrl,
        Vouch,
    },
...
mod tests {
    ...
    fn test_simple_vp() {
        ...
        assert_eq!(vp, vp_new);
        let vp_new = VP::from_vouch_with_type_contexts(
            &vouch,
            HashMap::from([(
                "BosonProfile",
                vec!["https://example.com/credentials/profile/v1".to_string()],
            )]),
        );
        assert_eq!(vp, vp_new);
    ...
    fn test_complex_vp() {
        ...
        let mut expected = vec![constants::DEFAULT_VP_TYPE, "TestPresentation"];
        expected.sort();
        ...
        let vp_new = VP::from_vouch_with_type_contexts(
            ...
            HashMap::from([
                ...
                (
                    "Passport",
                    vec!["https://example.com/credentials/passport/v1".to_string()],
                ),
                (
                    "Email",
                    vec!["https://example.com/credentials/email/v1".to_string()],
                ),
                (
                    "DriverLicense",
                    vec!["https://example.com/credentials/driverLicense/v1".to_string()],
                ),
            ]),
        );
        assert_eq!(vp, vp_new);
        ...
        let vp_new = VP::from_vouch_with_type_contexts(
            ...
            HashMap::from([
                (
                    "TestPresentation",
                    vec!["https://example.com/presentations/test/v1".to_string()],
                ),
                (
                    "BosonProfile",
                    vec!["https://example.com/credentials/profile/v1".to_string()],
