...
fn test_options_with_funs() {
    ...
    let peerid_options = Options::new(peerid, userid).with_peerid(replacement_peerid);
    assert_eq!(peerid_options.service_peerid(), &replacement_peerid);
    assert_eq!(peerid_options.service_peer(), None);

    let generated_user_options =
        Options::new(peerid, userid).with_generated_user_key();
    let generated_user_key = generated_user_options.user_key().unwrap();
    assert_eq!(
        generated_user_options.user_id(),
        &Id::from(generated_user_key.public_key())
    );

    let generated_device_options =
        Options::new(peerid, userid).with_generated_device_key();
...
fn test_load_then_overriden() {
    ...
    assert_eq!(
        options.user_id(),
        &Id::from(replacement_user_key.public_key())
    );
    assert_eq!(
        options.user_key().unwrap().private_key(),
    ...
    assert_eq!(
        options.device_key().private_key(),
...
fn test_load_and_expand_envioroment_variables() {
    ...
    let device_key = KeyPair::random();
    let variables = [
        ("ACTIVEPROXY_OPTIONS_TEST_PEERID", service_peerid.to_string()),
        (
