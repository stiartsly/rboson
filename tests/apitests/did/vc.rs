use boson::{
    did::{constants, w3c::VerifiableCredential as VC, DIDUrl},
...
mod tests {
    ...
    fn test_simple_vc() {
        ...
        let canonical_id = DIDUrl::new(identity.id(), None, None, Some("test"));
        ...
        types.sort();
        let mut expected = vec![constants::DEFAULT_VC_TYPE, "Passport"];
        expected.sort();
    ...
    fn test_complex_vc() {
        ...
        let _canonical_id = DIDUrl::new(identity.id(), None, None, Some("fullProfile"));
        ...
        let mut expected = vec![constants::DEFAULT_VC_TYPE, "Profile", "Passport", "Email"];
        expected.sort();
        assert_eq!(types, expected);
        ...
        assert_eq!(
            subject.claim::<String>("email"),
            Some("cV9dX@example.com".to_string())
        );
        assert_eq!(
            subject.claim::<String>("passport"),
            Some("123456789".to_string())
        );
        assert_eq!(
            subject.claim::<String>("phone"),
            Some("+1-123-456-7890".to_string())
        );
        assert_eq!(
            subject.claim::<String>("address"),
