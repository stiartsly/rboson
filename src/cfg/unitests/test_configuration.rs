...
use crate::{
    Id,
    cfg::{config::DEFAULT_DHT_PORT, configuration::Configuration},
...
mod tests {
    ...
    fn test_nodeconfig() {
        ...
        let device_keypair = KeyPair::random();
        ...
        let cfg = cfg
            ...
            .with_devmode(true)
            .with_device_key(device_keypair.private_key().clone())
            .build()
            .unwrap();
    ...
    fn test_activeproxyconfig() {
        ...
        let cfg = cfg
            ...
            .with_device_key(proxy_keypair.private_key().clone())
        ...
        assert_eq!(
            cfg.server_peerid().unwrap().to_string(),
            server_peerid.to_string()
        );
        assert_eq!(cfg.device_key(), Some(proxy_keypair.private_key()));
    ...
    fn test_yaml_configuration() {
        ...
        let device_key = KeyPair::random().private_key().to_string();
        ...
        assert_eq!(
            cfg.device_key(),
            Some(&PrivateKey::try_from(device_key.as_str()).unwrap())
        );
        ...
        assert_eq!(
            cfg.server_peerid().unwrap().to_string(),
            "5vVM1nrCwFh3QqAgbvF3bRgYQL5a2vpFjngwxkiS8Ja6"
        );
    ...
    fn test_load_configuration() {
        let keypair = KeyPair::random();
        let private_key = keypair.private_key().to_string();
        let yaml_device_key = KeyPair::random().private_key().to_string();
        let temp_dir = make_temp_dir("cfg-combined");
        let path = temp_dir.join("node.yaml");

        let yaml = format!(
            "ipv4: true\nport: 39001\nprivateKey: \"{private_key}\"\ndeviceKey: \"{yaml_device_key}\"\ndataDir: ./yaml-data\ndatabaseUri: sqlite://node.db\nlogConsole: false\n"
        );
        fs::write(&path, yaml).unwrap();
        ...
        let cfg = cfg
            ...
            .with_device_key(proxy_sk.clone())
        ...
        assert_eq!(cfg.device_key(), Some(&proxy_sk));
    ...
    fn test_build_node_and_activeproxy_options() {
        ...
        let cfg = cfg
            ...
            .with_log_console(true)
            .with_activeproxy_service_peerid(server_peerid)
            .with_user_key(user_keypair.private_key().clone())
            .with_device_key(proxy_keypair.private_key().clone())
        ...
        assert_eq!(
            ap_options.service_peerid().to_string(),
            server_peerid.to_string()
        );
        assert_eq!(
            ap_options.user_key().unwrap().private_key(),
            user_keypair.private_key()
        );
        assert_eq!(
            ap_options.device_key().private_key(),
            proxy_keypair.private_key()
        );
