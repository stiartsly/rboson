...
use crate::{dht::Node, errors::StateError, Id, PeerInfo, Result};

pub(crate) async fn lookup_peer(node: Arc<Node>, peerid: &Id) -> Result<Option<PeerInfo>> {
    debug!(
        "ActiveProxy is trying to find peer {} via DHT network...",
        peerid
    );
...
pub(crate) fn save_peer(path: &Path, peer: &PeerInfo) {
    debug!(
        "ActiveProxy is trying to persist peer {} into cached file...",
        peer.id()
    );
