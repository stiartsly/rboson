...
pub(crate) fn serialize_nonce_opt<S>(nonce: &Option<Vec<u8>>, se: S) -> StdResult<S::Ok, S::Error>
where S: Serializer,
{
    serialize_bytes_opt(nonce, se)
}

pub(crate) fn deserialize_nonce_opt<'de, D>(de: D) -> StdResult<Option<Vec<u8>>, D::Error>
where D: Deserializer<'de>,
{
    Option::<Vec<u8>>::deserialize(de)
}

pub(crate) fn serialize_sig<S>(sig: &Vec<u8>, se: S) -> StdResult<S::Ok, S::Error>
where S: Serializer,
{
    serialize_bytes(sig, se)
}

pub(crate) fn serialize_sig_opt<S>(sig: &Option<Vec<u8>>, se: S) -> StdResult<S::Ok, S::Error>
where S: Serializer,
{
    serialize_bytes_opt(sig, se)
}

pub(crate) fn deserialize_sig<'de, D>(de: D) -> StdResult<Vec<u8>, D::Error>
where D: Deserializer<'de>,
{
    Vec::<u8>::deserialize(de)
}

pub(crate) fn deserialize_sig_opt<'de, D>(de: D) -> StdResult<Option<Vec<u8>>, D::Error>
where D: Deserializer<'de>,
{
    Option::<Vec<u8>>::deserialize(de)
}
...
pub(crate) fn serialize_seq<S>(seq: &i32, se: S) -> StdResult<S::Ok, S::Error>
where S: Serializer,
{
    if se.is_human_readable() {
...
pub(crate) fn serialize_expected_seq<S>(expected_seq: &i32, se: S) -> StdResult<S::Ok, S::Error>
where S: Serializer,
{
...
pub(crate) fn serialize_count<S>(count: &i32, se: S) -> StdResult<S::Ok, S::Error>
where S: Serializer,
{
    if se.is_human_readable() {
