use crate::{
    dht::msg::{FindNodeResponse, LookupResponse},
    Id, Network, NodeInfo,
};
...
mod tests {
    ...
    fn test_with_nodes() {
        ...
        let rsp = FindNodeResponse::new(Some(vec![node4.clone()]), None, 0);
        ...
        let rsp = FindNodeResponse::new(None, Some(vec![node6.clone()]), 0);
        ...
        let node4 = make_node_info4();
        let node6 = make_node_info6();
        let rsp = FindNodeResponse::new(Some(vec![node4.clone()]), Some(vec![node6.clone()]), 1);
    ...
    fn test_serde() {
        ...
        let rsp =
            FindNodeResponse::new(Some(vec![node4.clone()]), Some(vec![node6.clone()]), token);

        let necoded = serde_cbor::to_vec(&rsp).expect("Serialization failed");
        let decoded =
            serde_cbor::from_slice::<FindNodeResponse>(&necoded).expect("Deserialization failed");
    ...
    fn test_serde_with_nodes() {
        ...
        let token = 12345;
        let rsp = FindNodeResponse::new(Some(vec![node1.clone(), node2.clone()]), None, token);
    ...
    fn test_serde_without_token() {
        ...
        let encoded = serde_cbor::to_vec(&serde_cbor::Value::Map(
            vec![(
                ...
                serde_cbor::value::to_value(vec![node.clone()]).expect("Failed to encode nodes"),
            )]
            .into_iter()
            .collect(),
        ))
