use crate::director::{
    ...
    DirectorClient, ProfileUpdate, UserRegistration,
};
...
fn builder_rejects_incomplete_device_identity() {
    ...
    builder
        .with_director_url("https://director.example")
        .unwrap();
    builder.with_device_key(device);
...
fn builder_derives_user_and_device_ids() {
    ...
    let mut builder = DirectorClient::builder();
    builder
        .with_director_url("https://director.example/prefix/")
        .unwrap();
    builder.with_user_key(user).with_device_key(device);
